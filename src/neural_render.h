...
public:
  ...
  void Compute(OpKernelContext *context) override {
    ...
    impl(batch_size, np, nf, pts_data, faces_data, uvs_data, H, W,
...
template <typename Device> class RasterizeGradOp : public OpKernel {
  static void impl(int batch_size, int nfaces, int npoints, int H, int W,
                   const float *pts_data, const int32_t *faces_data,
...
public:
  ...
  void Compute(OpKernelContext *context) override {
    ...
    impl(batch_size, nf, np, H, W, pts_data, faces_data, uvs_data,
         out_fids_data, out_bc_data, grad_uvgrid_data, grad_z_data,
         grad_pts_data);
...
public:
  ...
  void Compute(OpKernelContext *context) override {
    LOG(INFO) << "begin bilinear_sample_grad";
    const Tensor &tex = context->input(0); // BxHtxWtxDt
    auto tex_data = tex.flat<float>().data();
    const Tensor &uvgrid = context->input(1); // BxHxWx2, float
    auto uvgrid_data = uvgrid.flat<float>().data();

    const Tensor &grad_sampled = context->input(2); // BxHxWxDt, float
    auto grad_sampled_data = grad_sampled.flat<float>().data();
    ...
    Tensor *grad_tex_ptr = nullptr;
    Tensor *grad_uvgrid_ptr = nullptr;
    OP_REQUIRES_OK(context,
                   context->allocate_output(
                       0, TensorShape{batch_size, Ht, Wt, Dt}, &grad_tex_ptr));
    OP_REQUIRES_OK(context,
                   context->allocate_output(1, TensorShape{batch_size, H, W, 2},
                                            &grad_uvgrid_ptr));
    auto grad_tex_data = grad_tex_ptr->flat<float>().data();
    auto grad_uvgrid_data = grad_uvgrid_ptr->flat<float>().data();

    impl(batch_size, Ht, Wt, Dt, H, W, tex_data, uvgrid_data, grad_sampled_data,
         grad_tex_data, grad_uvgrid_data);
    LOG(INFO) << "done bilinear_sample_grad";
  }
};
